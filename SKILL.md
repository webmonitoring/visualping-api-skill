...
- **Rate limits**: some endpoints return HTTP 429 (`THROTTLED`) when called too fast. The one users hit most is diff fetching — `GET /v2/jobs/get-diff`, the endpoint behind diff links, allows 200 requests per rolling minute per caller. Generated code that loops over many jobs or changes should pace its requests and back off exponentially on 429. Sustained over-limit calling can get an API key or account temporarily suspended, so for legitimate high-volume needs point the user to support@visualping.io instead of retrying harder. See the Rate Limits section of the reference.
